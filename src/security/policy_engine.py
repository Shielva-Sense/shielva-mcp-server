...
class OPAPolicyEngine:
    ...
    async def evaluate(self, context: PolicyContext) -> PolicyDecision:
        ...
        try:
            ...
            input_data = {
                ...
                "role": context.user_role,
                "action": context.action,
                "resource": {"type": context.resource_type, "id": context.resource_id},
                "metadata": context.metadata or {},
            }

            # Query OPA
            result = await self._query_opa(f"{self.policy_path}/authz/allow", input_data)
            ...
            if not self._opa_warned:
                logger.warning(
                    "OPA unavailable — using fallback RBAC for this and subsequent requests",
                    error=str(e),
                )
            ...
            # Fail closed
            return PolicyDecision(allowed=False, reason=f"Policy evaluation failed: {e!s}")
    ...
    async def check_quota(
        ...
        try:
            ...
            result = await self._query_opa(f"{self.policy_path}/quota/allow", input_data)
    ...
    async def get_data_filters(
        self, tenant_id: str, user_id: str, user_role: str, resource_type: str
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            result = await self._query_opa(f"{self.policy_path}/filter/data", input_data)
    ...
    async def check_feature(self, tenant_id: str, user_role: str, feature: str) -> bool:
        """
        ...
        try:
            input_data = {"tenant_id": tenant_id, "role": user_role, "feature": feature}

            result = await self._query_opa(f"{self.policy_path}/features/enabled", input_data)
    ...
    async def _query_opa(self, path: str, input_data: dict[str, Any]) -> dict[str, Any]:
        """Query OPA endpoint"""
    ...
    def _fallback_evaluate(self, context: PolicyContext) -> PolicyDecision:
        ...
        if role not in self._role_permissions:
            return PolicyDecision(allowed=False, reason=f"Unknown role: {role}")

        permissions = self._role_permissions[role]

        if resource_type not in permissions:
            return PolicyDecision(allowed=False, reason=f"No access to resource type: {resource_type}")
    ...
    def _fallback_quota_check(self, user_role: str, resource_type: str, current_count: int) -> PolicyDecision:
        """Fallback quota check"""
        quotas = self._default_quotas()

        if user_role not in quotas:
            return PolicyDecision(allowed=False, reason=f"Unknown role: {user_role}")
        ...
        if current_count < limit:
            return PolicyDecision(
                allowed=True,
                conditions={"limit": limit, "remaining": limit - current_count - 1},
            )

        return PolicyDecision(
            allowed=False,
            reason=f"Quota exceeded: {current_count}/{limit} {resource_type}s",
            conditions={"limit": limit, "remaining": 0},
        )

    def _fallback_feature_check(self, user_role: str, feature: str) -> bool:
