...
class PolicyApplicationService:
    ...
    async def require_permission(
        ...
        *,
        ...
        if not decision.allowed:
            logger.warning(
                "mcp.policy_denied",
                tenant_id=tenant.tenant_id, action=action,
                resource_type=resource_type, resource_id=resource_id,
                reason=decision.reason,
            )
            raise PolicyDeniedError(
                f"{action} on {resource_type} denied: "
                f"{decision.reason or 'no reason given'}"
            )
        return decision

    async def check_quota(self, *, tenant: TenantContext,
                          resource_type: str,
    ...
    async def check_feature(self, *, tenant: TenantContext,
                            feature: str) -> FeatureDecision:
